...
BOOLEAN QuerySymbolicLink(PUNICODE_STRING usName, PUNICODE_STRING usDestination)
{
    ...
    HANDLE h = NULL;
    OBJECT_ATTRIBUTES ObjAttr;
    InitializeObjectAttributes(&ObjAttr, usName, OBJ_CASE_INSENSITIVE | OBJ_KERNEL_HANDLE, NULL, 0);            


    NTSTATUS ns = ZwOpenSymbolicLinkObject(&h, GENERIC_READ, &ObjAttr);
    if (NT_SUCCESS(ns))
    {
        USHORT wSize = 255 * sizeof(WCHAR);
query:

        PWSTR Buff = (PWSTR)M_ALLOC(wSize);
        if (Buff)
        {
            ULONG RetLen = 0;

            usDestination->Buffer = Buff;
            usDestination->Length = 0;
            usDestination->MaximumLength = wSize;

            ns = ZwQuerySymbolicLinkObject(h, usDestination, &RetLen);
            if (NT_SUCCESS(ns))
            {
                bRet = TRUE;
            }
            else if (ns == STATUS_BUFFER_TOO_SMALL)
            {
                M_FREE(Buff);
                wSize += 0x100;
                goto query;
            ...
            {
                M_FREE(Buff);
                DbgMsg(__FILE__, __LINE__, "NtQuerySymbolicLinkObject() fails; status: 0x%.8x\n", ns);
            }
        }
        else
        {
...
#define	PROCESS_PARAMETERS_NORMALIZED	1	
